#!/usr/bin/env python3
"""
Ollama Security Auditor Final Edition
Security testing framework with IP Range Scanning, Model Discovery & FULL Prompt Extraction.
Features:
...
- 📄 FULL Prompt Extraction: Extracts System, Template, Parameters, Modelfile.
...
def sanitize_version(version_str: str) -> str:
    """Sanitize version string using a safe character whitelist to prevent CRLF & breakout."""
    if not isinstance(version_str, str):
        return "unknown"
    return "".join(c for c in version_str if c.isalnum() or c in ".-_")

def sanitize_model_name(name_str: str) -> str:
    """Sanitize model name using a safe character whitelist to prevent directory traversal & breakout."""
    if not isinstance(name_str, str):
        return "unknown"
    # Keep only alphanumeric, dot, hyphen, underscore, colon, forward-slash
    cleaned = "".join(c for c in name_str if c.isalnum() or c in ".-_:/")
    # Defensively replace double dots to prevent path traversal at the sanitization layer
    while ".." in cleaned:
        cleaned = cleaned.replace("..", "_")
    return cleaned

def sanitize_digest(digest_str: str) -> str:
    """Sanitize digest blob using a safe character whitelist to prevent injection."""
    if not isinstance(digest_str, str):
        return ""
    return "".join(c for c in digest_str if c.isalnum() or c in ":-_")
...
class OllamaSecurityAuditor:
    ...
    async def _fetch_github_advisories(self, session: aiohttp.ClientSession) -> List[Dict[str, Any]]:
        """Fetch live advisories from GitHub Security advisories API securely via safe_request."""
        results = []
        try:
    ...
    async def _fetch_nvd_advisories(self, session: aiohttp.ClientSession) -> List[Dict[str, Any]]:
        """Fetch live advisories from NVD CVE Search API securely via safe_request."""
        results = []
        try:
    ...
    async def _fetch_exploitdb_advisories(self, session: aiohttp.ClientSession) -> List[Dict[str, Any]]:
        """Fetch live advisories from ExploitDB API securely via safe_request."""
        results = []
        try:
        ...
        if s == 200 and b and "models" in b:
            ...
            for m in b["models"]:
                if isinstance(m, dict):
                    san_m = m.copy()
                    san_m["name"] = sanitize_model_name(m.get("name", "unknown"))
                    self.loaded_models.append(san_m)
    ...
    def _clean_modelfile(self, modelfile: str) -> str:
        """Remove any license block starting with 'LICENSE' on its own line."""
        # Find the first occurrence of LICENSE at the beginning of a line
        match = re.search(r'(^|\n)LICENSE\b', modelfile)
        if match:
            # Remove everything from that position to the end
            modelfile = modelfile[:match.start()]
        return modelfile.rstrip()

    async def extract_model_configs(self, session: aiohttp.ClientSession) -> List[AuditFinding]:
        """Extract FULL model configurations and save to human-readable .md files"""
        ...
        for model in models:
            ...
            if status == 200 and config_body:
                ...
                template = str(config_body.get("template", ""))
                parameters = str(config_body.get("parameters", ""))
                modelfile_raw = str(config_body.get("modelfile", ""))

                # Clean the Modelfile by removing any license block
                ...
                try:
                    ...
                    for pat in sensitive_patterns:
                        if re.findall(pat, full_context, re.IGNORECASE):
                            matches.append(pat)

                    details = "Full config extracted. Sensitive patterns found." if matches else "Full config extracted."
                    ...
                    findings.append(AuditFinding(
                        check_name=f"Extraction Error ({model_name})",
                        severity=Severity.HIGH,
                        status=CheckStatus.ERROR,
                        details=f"Failed to save configuration: {e}",
    ...
    async def run_audit(self, session: aiohttp.ClientSession) -> List[AuditFinding]:
        ...
        await self.discover_models(session)
...
def main():
    parser = argparse.ArgumentParser(
        description="Ollama Security Auditor Final Edition - Range Scanning & Advanced Probes",

#!/usr/bin/env python3
"""
Ollama Security Auditor v1.4
Security testing tool for exposed Ollama API endpoints (default port 11434)
Features:
- Robust Markdown & JSON report generation.
- Extracts and displays the actual detected version from /api/version.
- Automatic URL correction (http:// + :11434) with strict scheme validation.
- Comprehensive CVE checks based on detected version + Dynamic Advisory Integration.
- Rate Limit & WAF Detection with auto-throttling.
- Advanced Probes: Model Weight Exfiltration, Streaming DoS, Modelfile RCE, Prompt Injection, Cloud Metadata SSRF, Token Brute-Force.
...
class CustomEncoder(json.JSONEncoder):
    ...
    Converts Enums to their value strings.
    """
...
def resolve_target_url(target: str) -> str:
    """
    Fixes URL format error: Ensures 'http://' and ':11434' are explicitly present.
    Adds strict scheme validation to prevent SSRF/file:// abuse.
    Example output: http://192.168.1.100:11434
    """
    target = target.strip().rstrip('/')
    
    # 1. Ensure Protocol & Validate Scheme
    ...
    if path_sep_index != -1:
        host_part = rest[:path_sep_index]
        path_part = rest[path_sep_index:]
    else:
        host_part = rest
        path_part = ""
...
def _parse_version_tuple(ver_str: str) -> Tuple[int, ...]:
    """Safely parse semantic version strings to comparable tuples, ignoring pre-release/build metadata."""
    clean = re.sub(r'[^0-9.]', '', ver_str.split('-')[0])
    parts = clean.split('.')
    # Pad to at least 3 parts for consistent comparison (e.g., 0.10 vs 0.9)
    while len(parts) < 3:
        parts.append('0')
...
def _version_in_range(target_ver: str, range_str: str) -> bool:
    """Check if target version falls within an affected range (e.g., '>=0.3.0,<0.4.2')"""
...
# Curated CVE database for Ollama (Static Fallback)
CVE_REGISTRY: List[Dict[str, Any]] = [
    {
        "cve_id": "CVE-2024-37032",
        "title": "ProbLlama: Critical Path Traversal & SSRF",
        "severity": Severity.CRITICAL,
        "affected_range": ">=0.1.0,<0.3.14",
        "check_type": "endpoint_version_match",
        "description": "Wiz research discovered critical path traversal and SSRF via /api/pull.",
        "remediation": "Upgrade to >=0.3.14. Block /api/pull at reverse proxy.",
        "indicator": "Version <0.3.14"
    },
    {
        ...
        "remediation": "Upgrade to >=0.3.14. Set OLLAMA_HOST=127.0.0.1.",
        "indicator": "Version <0.3.14"
    },
    {
        "cve_id": "CVE-2024-39722",
        "title": "SSRF via Model Pull",
        "severity": Severity.HIGH,
        "affected_range": ">=0.1.0,<0.3.14",
        "check_type": "endpoint_version_match",
        "description": "Improper URL validation in /api/pull allows SSRF.",
        "remediation": "Upgrade to >=0.3.14.",
        "indicator": "Version <0.3.14"
    },
    {
        "cve_id": "CVE-2024-45436",
        "title": "Path Traversal via Modelfile",
        "severity": Severity.CRITICAL,
        "affected_range": ">=0.1.0,<0.4.0",
        "check_type": "endpoint_version_match",
        "description": "Traversing directory boundaries in Modelfile FROM paths.",
        "remediation": "Upgrade to >=0.4.0.",
        "indicator": "Version <0.4.0"
    },
    {
        "cve_id": "CVE-2024-7773",
        "title": "Insecure Unix Socket Permissions",
        "severity": Severity.HIGH,
        "affected_range": ">=0.1.0,<0.5.0",
        "check_type": "endpoint_version_match",
        "description": "Default permissions allow local unprivileged access.",
        "remediation": "Upgrade to >=0.5.0.",
        "indicator": "Version <0.5.0"
    },
    {
        "cve_id": "CVE-2025-15514",
        "title": "API Parameter Leakage",
        "severity": Severity.HIGH,
        "affected_range": ">=0.5.0,<0.6.0",
        "check_type": "behavioral_probe",
        "description": "Improper validation of chat parameters leaks context.",
        "remediation": "Upgrade to >=0.6.0.",
        "indicator": "Version range match"
    },
    {
        "cve_id": "GHSA-x9hg-5q6g-q3jr",
        ...
        "indicator": "Error verbosity"
    },
    {
        "cve_id": "SNYK-WOLFILATEST-OLLAMA-6035174",
        "title": "Dependency Chain Risk",
        "severity": Severity.MEDIUM,
        "affected_range": ">=0.1.0,<0.4.0",
        "check_type": "endpoint_version_match",
        "description": "Underlying dependency flaws in model processing.",
        "remediation": "Upgrade Ollama binary.",
        "indicator": "Version <0.4.0"
    }
]
...
class OllamaSecurityAuditor:
    ...
    async def check_connectivity(self, session: aiohttp.ClientSession) -> AuditFinding:
        ...
        if status == 200 and body and "version" in body:
            ...
            return AuditFinding(
                check_name="API Connectivity & Version Detection",
                ...
                evidence={"version": self.detected_version}
            )
        return AuditFinding(
            check_name="API Connectivity & Version Detection",
            severity=Severity.HIGH,
    ...
    async def _fetch_dynamic_advisories(self, session: aiohttp.ClientSession):
        """
        Fetch live advisories from multiple sources:
        1. GitHub Security Advisories (Primary)
        2. NVD API (CVE Database)
        3. ExploitDB Search (Public Mirror)
        Merges results into _dynamic_advisories_cache with deduplication.
        """
        ...
        try:
            ...
            headers = {"Accept": "application/vnd.github+json", "User-Agent": "OllamaAuditor/1.4"}
                    ...
                    for adv in data:
                        ...
                        if ghsa not in seen_ids:
                            ...
                            self._dynamic_advisories_cache.append({
                                "cve_id": ghsa,
                                "title": adv.get("summary", "GitHub Advisory"),
                                "severity": Severity.HIGH,
                                ...
                                "indicator": "GitHub Advisory Match",
                                "source": "GitHub"
                            })
        except Exception as e:
            logger.debug(f"GitHub advisory fetch failed: {e}")
        ...
        try:
            ...
            logger.debug(f"NVD advisory fetch failed: {e}")

        # 3. ExploitDB Search (Public JSON Feed)
        try:
            edb_url = "https://www.exploit-db.com/api/v1/exploits?search=ollama&pageSize=5"
            headers = {"Accept": "application/json", "User-Agent": "OllamaAuditor/1.4"}
                ...
                if resp.status == 200:
                    ...
                    for exp in data.get("data", []):
                        ...
                        if edb_id not in seen_ids:
                            ...
                            self._dynamic_advisories_cache.append({
                                "cve_id": edb_id,
                                "title": f"ExploitDB: {exp.get('title', 'Unknown Exploit')}",
                                "severity": Severity.HIGH,
                                "affected_range": ">=0.0.0",
                                "check_type": "endpoint_version_match",
                                ...
                                "indicator": "ExploitDB Match",
                                "source": "ExploitDB"
                            })
        except Exception as e:
            logger.debug(f"ExploitDB advisory fetch failed: {e}")
    ...
    async def check_known_cves(self, session: aiohttp.ClientSession) -> List[AuditFinding]:
        ...
        # Merge static + dynamic advisories
        ...
        for cve in combined_registry:
            ...
            if status != CheckStatus.SECURE:
                cve_findings.append(AuditFinding(
                    check_name=f"CVE Check: {cve['title']}",
                    severity=cve["severity"],
                    status=status,
                    details=f"{cve['description']}{suffix}",
    ...
    async def check_authentication(self, session: aiohttp.ClientSession) -> AuditFinding:
        ...
        if accessible:
            return AuditFinding(
                check_name="Authentication Bypass",
                severity=Severity.CRITICAL,
                status=CheckStatus.VULNERABLE,
        ...
        return AuditFinding(
            ...
            status=CheckStatus.SECURE,
            details="Endpoints returned non-200 status.",
            remediation="Verify protection."
        )
            ...
            async with session.get(
                ...
                if "*" in allow_origin or malicious_origin in allow_origin:
                    return AuditFinding(
                        ...
                        remediation="Set OLLAMA_ORIGINS explicitly.",
                        evidence={"allow_origin": allow_origin}
                    )
        except Exception:
            pass
        return AuditFinding(
    ...
    async def check_dangerous_endpoints(self, session: aiohttp.ClientSession) -> AuditFinding:
        ...
        for endpoint, method in dangerous_eps.items():
            ...
            # FIX: Only flag if endpoint is reachable AND not explicitly blocked by auth/method
            if status not in [401, 403, 405, 502]:
                exposed.append(f"{endpoint} ({method})")
                
        if exposed:
            return AuditFinding(
                check_name="Dangerous Endpoint Exposure",
                severity=Severity.CRITICAL if not self.deep_mode else Severity.HIGH,
                status=CheckStatus.VULNERABLE,
                details=f"Destructive/modify endpoints accessible: {', '.join(exposed)}.",
        ...
        return AuditFinding(
            ...
            severity=Severity.LOW,
            status=CheckStatus.SECURE,
            details="Destructive endpoints appear restricted.",
    ...
    # ==============================================================================
    # NEW FEATURES & ADVANCED PROBES
    # ==============================================================================
    async def check_waf_rate_limit(self) -> AuditFinding:
        """Check for WAF presence and rate limiting headers"""
        details = []
        if self._waf_detected:
            details.append("WAF/Proxy signatures detected in response headers.")
        if self._rate_limit_detected:
            details.append("Rate limiting headers present (auto-throttle active).")
            
        if details:
            return AuditFinding(
                check_name="WAF & Rate Limit Detection",
                severity=Severity.INFO,
                status=CheckStatus.SECURE,
                details="; ".join(details),
                remediation="Monitor for aggressive throttling during deep scans.",
    ...
    async def check_model_weight_exfil(self, session: aiohttp.ClientSession) -> AuditFinding:
        ...
        if b_status == 200:
            return AuditFinding(
                check_name="Model Weight Exfiltration",
                severity=Severity.CRITICAL,
                status=CheckStatus.VULNERABLE,
    ...
    async def check_modelfile_rce(self, session: aiohttp.ClientSession) -> AuditFinding:
        ...
        if status == 200:
            return AuditFinding(
                check_name="Modelfile RCE Probe",
                severity=Severity.CRITICAL,
                status=CheckStatus.VULNERABLE,
            ...
            return AuditFinding(
                ...
                remediation="Maintain current validation.",
                evidence={"error": body.get("error")}
            )
        return AuditFinding(
            check_name="Modelfile RCE Probe",
            severity=Severity.INFO,
            status=CheckStatus.SKIPPED,
            details="Endpoint unreachable or unexpected status.",
    ...
    async def check_prompt_injection_leakage(self, session: aiohttp.ClientSession) -> List[AuditFinding]:
        ...
        for model in body.get("models", [])[:3]:
            ...
            if cfg and "system" in cfg:
                ...
                if matches:
                    findings.append(AuditFinding(
                        check_name=f"Prompt Injection Leakage ({m_name})",
                        severity=Severity.HIGH,
                        status=CheckStatus.VULNERABLE,
    ...
    async def check_cloud_metadata_ssrf(self, session: aiohttp.ClientSession) -> AuditFinding:
        ...
        for meta_url in metadata_urls:
            ...
            if s == 200 or (b and any(k in str(b) for k in ["ami-id", "instance-id", "google"])):
                return AuditFinding(
                    check_name="Cloud Metadata SSRF",
                    severity=Severity.CRITICAL,
                    status=CheckStatus.VULNERABLE,
    ...
    async def check_token_brute(self, session: aiohttp.ClientSession) -> AuditFinding:
        ...
        for token in common_tokens:
            ...
            if s == 200:
                ...
                break  # Stop after first success to avoid lockouts
                
        if vulnerable:
            return AuditFinding(
                check_name="Token/Key Brute-Force",
                severity=Severity.CRITICAL,
                status=CheckStatus.VULNERABLE,
        ...
        return AuditFinding(
            ...
            severity=Severity.LOW,
            status=CheckStatus.SECURE,
            details="Weak/default tokens rejected.",
            remediation="Verify credential rotation policies."
        )

    async def run_audit(self) -> List[AuditFinding]:
        ...
        async with aiohttp.ClientSession(
            connector=connector,
            headers={"User-Agent": "OllamaSecurityAuditor/1.4"}
        ) as session:
            # Phase 0: Dynamic Advisory Fetch (Multi-Source)
            await self._fetch_dynamic_advisories(session)
            
            # Phase 1: Connectivity & Version Detection
            ...
            # Phase 2: CVE Vulnerability Assessment (Static + Dynamic)
            ...
            # Phase 3: Concurrent independent checks
            auth_result, info_result, cors_result, dangerous_result = await asyncio.gather(
                self.check_authentication(session),
                self.check_info_disclosure(session),
                self.check_cors_policy(session),
                self.check_dangerous_endpoints(session)
            )
            self.findings.extend([auth_result, info_result, cors_result, dangerous_result])
            
            # Phase 4: Advanced Probes (Sequential for safety & deep mode gating)
            ...
            # Phase 5: Model Configuration Extraction
            ...
            for s in Severity:
                self.stats[s.value] = sum(1 for f in self.findings if f.severity == s)
        ...
        if format_type == 'json':
            ...
            report_data = {
                "metadata": {
                    ...
                    "ip_port": self.target_ip_display,
            ...
            with open(report_path, 'w', encoding='utf-8') as file:
                json.dump(report_data, file, indent=2, cls=CustomEncoder)
            return report_path
            
        elif format_type == 'md':
            report_path = f"{output_path}_audit_{timestamp}.md"
            lines = [
                f"# 🛡️ Ollama Security Audit Report",
                ...
                f"| **IP:PORT** | `{self.target_ip_display}` |",
                f"| **Detected Version** | `{self.detected_version}` |",
                f"| **Status** | ✅ AUDIT COMPLETED |",
                "",
                "## 📊 Summary Statistics",
...
def main():
    parser = argparse.ArgumentParser(
        description="Security auditor for exposed Ollama API instances (Port 11434)",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument("target", help="Target URL or IP (e.g., 192.168.1.50 or http://IP:11434)")
    parser.add_argument("-o", "--output", default="ollama_audit_report", help="Base name for report (generates .md or .json). Default: ollama_audit_report")
    ...
    print("=" * 70, file=sys.stderr)
    print("🛡️  OLLAMA SECURITY AUDITOR v1.4", file=sys.stderr)
    print("=" * 70, file=sys.stderr)
    print("Features: Markdown/JSON Reports, Version Extraction, Dynamic Multi-Source Advisories, Advanced Probes", file=sys.stderr)
    print("=" * 70, file=sys.stderr)
    
    try:
        findings = asyncio.run(auditor.run_audit())
    except KeyboardInterrupt:
        print("\n⚠️  Audit interrupted by user.", file=sys.stderr)
        sys.exit(130)
    except Exception as e:
    ...
    if not findings:
        print("\n⚠️  No findings generated.", file=sys.stderr)
        sys.exit(1)
        
    print(f"\n{'='*70}", file=sys.stderr)
    print(f"📊 AUDIT SUMMARY - {len(findings)} checks performed", file=sys.stderr)
    for sev in Severity:
        count = auditor.stats.get(sev.value, 0)
        if count > 0: print(f"  🔹 {sev.value}: {count}", file=sys.stderr)
    print(f"{'='*70}", file=sys.stderr)
    
    # Always generate report (default .md, overridable via --output)
